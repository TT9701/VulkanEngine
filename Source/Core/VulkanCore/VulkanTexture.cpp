...
//
// void VulkanTexture::CopyToImage(vk::CommandBuffer cmd, vk::Image dstImage,
...
VulkanTexture::VulkanTexture(VulkanDevice* device, vk::Image handle, Type type,
                             vk::Format format, vk::Extent3D extent,
                             uint32_t arraySize, uint32_t sampleCount)
    : pDevice(device),
      pAllocator(nullptr),
...
uint32_t VulkanTexture::GetHeight(uint32_t mipLevel) const {
    return (mipLevel == 0) || (mipLevel < mMipLevels)
             ? std::max(1U, mExtent3D.height >> mipLevel)
             : 0;
}

uint32_t VulkanTexture::GetDepth(uint32_t mipLevel) const {
    return (mipLevel == 0) || (mipLevel < mMipLevels)
             ? std::max(1U, mExtent3D.depth >> mipLevel)
             : 0;
}

uint32_t VulkanTexture::GetMipCount() const {
    return mMipLevels;
}
...
vk::Format VulkanTexture::GetFormat() const {
    return mFormat;
}

vk::Image VulkanTexture::GetHandle() const {
    return mHandle;
}
