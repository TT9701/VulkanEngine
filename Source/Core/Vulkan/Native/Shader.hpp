...
public:
    // from spirv
    Shader(Context* context, const char* spirvPath, ShaderStage stage,
           const char* entry = "main", void* pNext = nullptr);

    // from glsl source code
    Shader(Context* context, const char* sourcePath, ShaderStage stage,
...
public:
    vk::ShaderModule CreateShader(::std::span<uint32_t> binaryCode,
                                  void* pNext) const;
